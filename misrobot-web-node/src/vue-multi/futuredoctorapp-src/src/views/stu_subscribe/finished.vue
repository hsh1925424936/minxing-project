<template>
    <div>
        <div class="xiecontentBox" v-for="(value,index) in finishedList">
            <dl>
                <dd>
                    <span>实验室名称 : </span>
            ...
            <p class="xiebottomHandlers">
                <router-link :to="{name: 'fdstuscoreanalysis',query:{scoreid:value.id}}" class="xiebtn xiebtn-default">训练成绩</router-link>
            </p>
        </div>
    ...
    export default {
        ...
        mounted() {
            ...
            this.post('/learn/querystudentapplyinfobystatus', {
                ...
                loginid:$.cookie('uid'),
                status:1,//0：审核通过，1：已训练，2：训练失败，3：已取消,4：待审核（默认），5：审核不通过
                teacherid:$.cookie('uid')
	        }).done(function(data){console.log('---------------')
                self.finishedList=data.applys;
                console.log(JSON.stringify(self.finishedList));
	        }).fail(function () {
	        });
