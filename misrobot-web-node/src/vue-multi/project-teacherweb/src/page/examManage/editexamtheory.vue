<template>
	<div class="examtheory">
		...
		<div class="content">
			<p class="top">
				<span>考试名称：{{detexamname}}</span>
				<span>课程：{{detexamcoursename}}</span>
				<span>考试类型：{{detexamtype|typefn}}</span>
				<span>考试形式：{{detexammodel|modelfn}}</span>
			</p>
			<div class="bottom">
				<p class="cell_line">
					<span>考试名称:</span>
				...
				<p class="cell_line">
					<span>考试时间:</span>
					<el-date-picker v-model="timeval1" @change="stimechange" type="datetime" placeholder="开始时间" :picker-options="pickerOptions" class="cell_right" style="padding: inherit;width: 195px;"></el-date-picker>
					<el-date-picker v-model="timeval2" @change="etimechange" type="datetime" placeholder="结束时间" :picker-options="pickerOptions" class="cell_right" style="padding: inherit;width: 195px;"></el-date-picker>
				</p>
				<p class="cell_line">
					<span>考试试卷:</span>
				...
				<p class="cell_line">
					<span>考试地点:</span>
					<label class="cell_right box"><p v-for="item in siteselected">{{item.name}}</p></label><el-button type="primary" class="addbtn" @click="selectsite">选择</el-button>
				</p>
...
data() {
  	return {
		...
		exammodel:'',//考试形式
		detexamname:'',//考试名称
		detexamcoursename:'',//考试学科
		detexamtype:'',//考试类型
		detexammodel:'',//考试形式
		typelist:[{value:0,label:'单元考试'},{value:1,label:'期中考试'},{value:2,label:'期末考试'},{value:3,label:'随堂小测'}],
...
methods:{
	queryexamdetail(){//查询考试详情
		...
		post('/unifyexam/queryexamdetail',{
            ...
            if(res.errcode==0){
				...
				self.detexamname=res.name;
				self.detexamcoursename=res.coursename;
				self.detexamtype=res.type; 
				self.detexammodel=res.model;
            	self.examcourseid=res.courseid;
				self.timeval1=res.starttime;
				self.timeval2=res.endtime;
				...
				for(var i=0;i<res.placelist.length;i++){
					var obj={
						name:res.placelist[i].placename,
						id:res.placelist[i].placeid
					}
					self.siteselected.push(obj)
					self.value.push(obj.id)
				}
            }
        })
	},
	...
	save(){//保存考试
        ...
        if (self.startime==''&&self.endtime=='') {
        	this.starttime=self.time.slice(0,19);
			this.endtime=self.time.slice(22,41);
        }
        if(self.value.length==0){
        	for (var i=0;i<self.siteselected.length;i++) {
        		self.value.push(self.siteselected[i].id)	
        	}
        }
