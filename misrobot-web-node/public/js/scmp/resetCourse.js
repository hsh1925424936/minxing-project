...
(function(){//查询课程详细信息接口
    ...
    function callback(response){
        ...
        v.choosedPlace=response.roomnum;
        v.courseid=response.courseid;
        v.starttime=response.starttime;
        v.endtime=response.endtime;
        v.choosedCourseName=response.coursename;
        v.preExeSetted=response.tasks;
        v.choosedtimelist=response.timelist;
        console.log(v.preExeSetted[0]);
        for(index in response.teachers){
            if(response.teachers[index].role==3){//3表示授课老师
                v.choosedTeacherList.push({
                    teacherid:response.teachers[index].uid,
                    teacherame:response.teachers[index].name
                })
            }else{//教辅老师
        ...
        $("div.choosedTimeRange").html(response.starttime.slice(0,16)+"~"+response.endtime.slice(11,16));
    ...
    post("/course/querycoursedetailinfo",data,callback,errorfn,errorConnect)
})()
...
var v=new Vue({
    ...
    methods:{
        ...
        preExeSave:function(){
            ...
            // for(index in this.preExeSetted){
                console.log(JSON.stringify(this.preExeSetted))
            // };
        },
...
//          }
            ...
            if (self.checkList.length==1) {
                ...
                if(Date.parse(choosedStr)==Date.parse(currentStr)){
            			...
            			var newTime2=newCurrentStr2+" "+self.checkList[self.checkList.length-1].starttime;
        ...
        setStuSave:function(){//保存已选学生，并显示到页面
            ...
            if($("#studentList li.gray").length==0){
                ...
                self.stuList=[]
                // console.log(self.choosedsclasslist2)
                // self.choosedsclasslist2.map(item=>{
                //     item.stdlist.map(item1=>{
                //         studentList.push(item1.id)
                //     })
                // })
        ...
        saveAll:function(){//信息填完后点击保存按钮，新生成一节课
            var self=this;
            function callback(response){
                var L=layer.alert("修改课程成功！", {title:"成功提示",icon:1,btn:['我知道了']}, function () {
                    layer.close(L);
            ...
            if(self.placeid.length==0||self.courseid.length==0||self.teachers.length==0||self.starttime.length==0||self.endtime.length==0){
                ...
                // console.log(self.preExeSetted)
                // console.log('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
                // console.log(sendstudents)
                if(self.preExeSetted && self.preExeSetted.length == 0){
                    post("/course/updatecourseinfo",json,callback,errcode1,errorConnect)
                }else{
...
//课前训练模块
function preExeSearch(elem){//点击课前任务，选择训练项，显示下拉列表，显示所有训练项
...
function choosePreExe(elem){
    ...
    $("#preExeBox>p>span").html($(elem).html());
};
