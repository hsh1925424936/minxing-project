...
$(document).ready(function(e) {
	...
	//查询成长趋势
		...
		Api.R({
			url: "/learningindex/queryindexrank",
			isShade:1,
			json:{
				'1':{
					command:'learningindex/queryindexrank',
					accountname:getCookie('gy_un'),
					sessionid: getCookie('gy_t'),
					loginid: getCookie('gy_u'),
					uid: getCookie('gy_u')
	...
	Api.R({
		...
		fn: function(result) {
			...
			for (var i = _arr.length - 1; i >= 0; i--) {
				...
				var courseDay = (new Date(_arr[i].starttime)).getDay();

				_str+=	
						'<div class="clearfix">'
							+'<span class="clinicalSpan2 '+_timeStr1+'">'+_timeStr+'</span>'
							+'<span class="clinicalSpan3">'+_arr[i].starttime.substr(11,5)+'-'+_arr[i].endtime.substr(11,5)+'<br/>'+_arr[i].coursename+'</span>'
							+'<div class="clinicalRight">'
								+'<span class="clinicalRightSpan1">技能中心</span>'
								+'<span class="clinicalRightSpan2">' +_arr[i].roomnum+'室</span>'
								+'<span class="clinicalRightSpan3">'+_arr[i].teachers[0].name+'老师</span>'
							+'</div>'	
						+'</div>';
	...
	function getList(){
		Api.R({
			...
			fn:function (json) {
				...
				var　readMsg = '<img src="../../images/ostp/msg_readed.png">';
				var　newMsg = '<img src="../../images/ostp/new.png">';
				var  url = '';
				var  msgUrl = [];
				var reg = /http:\/\/[\w-\:\/?=&]*(\.[\w-\:\/?=&]*)+/ig;
				...
				if( arr.length > 6 ){
					for (var i = 0 ; i < 7; i++) {
						    msgUrl = arr[i].notecontent.match(reg);
						if( msgUrl ){
							url = msgUrl[0];
						}else {
							url = '/pc/see-msg.html';
