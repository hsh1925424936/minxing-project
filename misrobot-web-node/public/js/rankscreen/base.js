...
//	url:'http://192.168.1.166:8086',
	...
	R:function (options,url,type) {
		...
		if (!options.istoken) {
			if (getCookie("gy_t")) {
				options.json.access_token=getCookie("gy_t");
			} else {
				jumplogin();
				return false;
			}
		...
		if (options.islaravel) {
			url+='?access_token='+getCookie("gy_t");
			for (var name in options.json) {
				url+='&'+name+'='+options.json[name];
			}
			options.json={};
		}
		options.json._timeRandom = new Date().getTime()+Math.random();
...
function common(name) {
	...
	//$('body').append('<div class="beian">版权所有贵州医科大学\Powered by © 2015-2016 UI.CN</div>');
	...
	$('.headImg').attr('onerror', 'this.src="../img/head2.png"');
	$('.headImg2').attr('onerror', 'this.src="../img/head2.png"');
	$('.personImg').attr('onerror', 'this.src="../img/head2.png"');
...
function showpersoncenter(type) {
	...
	var json = {
		'个人资料':{
			...
			img:'../img/person-center-1'
		},
		'修改密码':{
			url:'change-pwd.html',
			img:'../img/person-center-2'
		},
		'查看通知':{
			url:'see-msg.html',
			img:'../img/person-center-3'
		},
		'我的预约':{
			url:'my-subscribe.html',
			img:'../img/person-center-4'
		},
		'已学内容':{
			url:'my-homework.html',
			img:'../img/person-center-5'
		},
		'训练曲线':{
			url:'train-record.html',
			img:'../img/person-center-6'
		},
		'技能课表':{
			url:'skill-table.html',
			img:'../img/person-center-7'
		},
		'查看评论':{
			url:'see-evalute.html',
			img:'../img/person-center-8'
		}
	};	
	for (var name in json) {
		var str = 
...
function subscribeLayer(txt,func){//取消预约弹出提示框
	$('body').append(
		'<div class="zhezhao">'+
			'<div class="warmOuter">'+
				'<img src="../img/closeLayer.png" class="cancel-icon" />'+
				'<h3 class="warm-top">'+
					'温馨提示'+
				'</h3>'+
				'<img src="../img/cancelLayer.png"/>'+
				'<h4>'+
					txt+
				'</h4>'+
