...
function show_report(base){//显示报告的所有内容
    (function(){//填写头部的基本信息
        if( base.classinfo.coursename ){
            ...
            $('#classnameID').html("<span>" + base.classinfo.classname+" " + base.classinfo.studentcount+"人</span>");
            $("#attendClassNum").html(base.classinfo.studentcount+'人');
    ...
    (function(){
        ...
        function show_err_details(){//操作错误详情
            $('#infoTd').html('<p class="no-data" style="height:166px">暂无相关数据</p>');
        ...
        function show_error_rate(){//显示错误率
            ...
            for ( var i = 0; i < base.errorratio.length; i++ ){
                ...
                if ( errorratio.traintype == $('.li-active').attr('traintype') && errorratio.trainitemname == $('.li-active').attr('trainitemname') ){
                    data_arr.push({value:(parseFloat(errorratio.errorratio)*100).toFixed(2) ,num:errorratio.errorcount});
