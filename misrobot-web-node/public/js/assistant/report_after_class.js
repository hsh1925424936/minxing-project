function set_default_report(){//页面加载显示报告的默认值
    $("#beforereport").html('');   //课程名称
    $('#reportdate').html('');  //当前时间
    $("#course_name").html('无');        //课程名
    $("#classnameID").html('无');     //班级和班级人数
    $('#attendClassNum').html('无');
    $("#attendClassTime").html('无');  //上课时间
    $('#attendance').html('无');
    $('#absenteeism').html('无');
    $('#absent_student').html('无');
    $("#reportdate").html('无');     //课程开始时间
}
...
function show_report(base){//显示报告的所有内容
    (function(){//填写头部的基本信息
        if( base.classinfo.coursename ){
            ...
            $('#reportdate').html( timetodate(new Date().getTime().toString().substr(0,10),4)  );
            $('#course_name').html(base.classinfo.coursename);
            $('#classnameID').html("<span>" + base.classinfo.classname+" " + base.classinfo.totalstudentcount+"人</span>");
            $("#attendClassNum").html(base.classinfo.totalstudentcount+'人');
    ...
    if( top.teacher_ability ){
        top.teacher_ability(base);
    }
    ...
    (function(){//学生评分evaluationitem
        ...
        if( base.evaluationmax == null ){
            $("#appraise").text("暂无分数");
        }else{
            $("#appraise").text(parseFloat(base.evaluationmax).toFixed(2));//学生评分:评价最高分
        }
    ...
    //         data_arr.push( parseFloat(classscore.averagescore).toFixed(2) );
    ...
    (function(){
        ...
        $('#menu li').click(function() {
            ...
            var trainitemname = $(this).attr('trainitemname');
            $('#menu').attr({
                traintype: traintype,
                trainitemname: trainitemname
            });
        ...
        if( !$('#menu').attr('traintype') ){
            $('#menu li').eq(0).click();
        }else{
            var traintype = $('#menu').attr('traintype');
            var trainitemname = $('#menu').attr('trainitemname');
            $('#menu li').each(function(i, el) {
                var _traintype = $(this).attr('traintype');
                var _trainitemname = $(this).attr('trainitemname');
                if( _traintype == traintype && _trainitemname == trainitemname ){
                    $(this).click();
                }
            });
        }
        ...
        function show_error_rate(){//显示错误率
            ...
            for ( var i = 0; i < base.errorratio.length; i++ ){
                ...
                if ( errorratio.traintype == $('.li-active').attr('traintype') && errorratio.trainitemname == $('.li-active').attr('trainitemname') ){
                    data_arr.push({value:(parseFloat(errorratio.errorratio)*100).toFixed(2) ,num:parseInt(errorratio.errorcount)});
                                ...
                                normal: {
                                    label : {
                                        ...
                                        formatter:function(a){
                                            return a.data.value+'% ,'+ a.data.num+'人'
                                        }
                                    }
                                }
                            },
            ...
            if ( data_arr.length == 0 ){
                ...
                var option = {
                    ...
                    xAxis : [
                        {
                            type : 'category',
                            //data : ['胃管链接错误', '洗胃溶液选择错误', '选择错误', '选择错误','选择错误'],
                            data:xAxis_arr,
                            axisLabel : {
                                interval : 0,
                                formatter : function(params){
        ...
        function show_ranklist(){//展示排行榜 studentrank
            //清除原来数据
            $('#ranking tr td:nth-child(1)').html('-');
            $('#ranking tr td:nth-child(2)').html('无');
            $('#ranking tr td:nth-child(3)').html('-');
            // for ( var i = 0; i < 7; i++ ){
            //     $('#ranking tr').eq(i+3).find('td').eq(0).html('-');
            // }
            ...
            if( studentrank_arr.length == 0 ){
                return false;
            }
            if ( studentrank_arr.length < 10 ){
                rankcount = studentrank_arr.length;
            }else{
                rankcount = 10;
            }
            for ( var i = 0; i < rankcount; i++ ){
                if ( studentrank_arr[i].myrank > 3 ){
                    $('#ranking tr').eq(i).find('td').eq(0).html(studentrank_arr[i].myrank);
                }else{
                    $('#ranking tr').eq(i).find('td').eq(0).html('<img src="../../images/assistant/paihang0'+studentrank_arr[i].myrank+'.png"/>');
                }
                $('#ranking tr').eq(i).find('td').eq(1).html(studentrank_arr[i].name);
                $('#ranking tr').eq(i).find('td').eq(2).html( parseFloat(studentrank_arr[i].score).toFixed(2) );
            }
        }

        function show_teachresult(){//教学成果对比
            $('#main10').html('');
            $('#main10').css('height','480px');
            $('#error_rate_num').html('教学成果对比');
            ...
            if ( yAxis_arr.length > 6 ){
                var content_height = 480 + 60*(yAxis_arr.length-6);
            ...
            if ( parseFloat(ratesum/ratenum).toFixed(2) > 0 ){
                $('#error_rate_num').html(
                    '教学成果对比（正确率平均提高了'+parseFloat(ratesum/ratenum).toFixed(2)+'%）'
                );
            }else{
                $('#error_rate_num').html('教学成果对比（平均正确率没有提升）');
            }
