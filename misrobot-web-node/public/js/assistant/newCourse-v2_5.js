...
var v=new Vue({
    ...
    created:function(){
        ...
        var loginid1=urlparam.split('&&')[1].split('=')[1]
        var sessionid1=urlparam.split('&&')[0].split('=')[1]
        setCookie('uid',loginid1)
        setCookie('sid',sessionid1)
    },
    mounted:function(){
        var self=this;
        $.ajax({
    ...
    methods:{
        savePreExeTime:function(){//保存最迟完成时间
            this.finishTime=preExeDate+" "+this.preendh+":"+this.preendmin+":"+"00";
        ...
        getPlaceList:function() {//获取授课地点的函数
            var self=this;
            self.showPlace=!self.showPlace;//从后台获取到授课地点后，把列表显示出来
            self.showCourseName=false;
            var json={
        ...
        saveAll:function(){//信息填完后点击保存按钮，新生成一节课
            ...
            function callback(response){
                var L=layer.alert("创建课程成功！", {title:"成功提示",icon:1,btn:['我知道了']}, function () {
                    layer.close(L);
                    window.parent.vm.today=false;
                    window.parent.vm.toweek=true;
                    window.parent.vm.initweek(response.scheduleid);
                    closeAll();
                });
            };
