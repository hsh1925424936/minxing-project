...
function jump_msxxq () { //点击消息提醒直接跳转到消息详情
    this.registerToNative('Notification', function (data) {

        var jsonStr = decodeURIComponent(JSON.parse(data).param);

        if(JSON.parse(jsonStr).category == "task"){
            window.location.href='todo.html';
        }else{
            window.location.href='msxxq.html?param='+encodeURIComponent(JSON.stringify(JSON.parse(jsonStr).param));
        }
    });
}

function queryMsgNum(){//查询消息条数
    if(GetQueryString('msgnum')){
        //vm.$set(vm.$data,"msgnum",GetQueryString('msgnum'));
        return GetQueryString('msgnum');
    }
    var data={
...
function queryTodoNum(){
    if(GetQueryString('todonum')){
        //vm.$set(vm.$data,"msgnum",GetQueryString('todonum'));
        return GetQueryString('todonum');
    }
    var data = {
    ...
    function callback (res) {
        var listnum = 0;
        if(!res.tasklist && !res.taskcountlist){
            setTimeout(function () {
                vm.$set(vm.$data,"todonum",0);
            },600);

        }else{
            var lab = 0;
            for(var i = 0; i < res.taskcountlist.length; i++){
                lab += res.taskcountlist[i].taskcount;
            }
            listnum = res.taskcount + lab;
            setTimeout(function () {
                vm.$set(vm.$data,"todonum",listnum);
            },600);
