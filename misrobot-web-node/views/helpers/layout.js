...
Handlebars.registerHelper({
	extend: function (partial, options) {
		...
		if(!template) {
			console.log('read : ' + `${config.view_dir}/${partial}${config.tpl_ext}`);
			var tmplSource = fs.readFileSync(`${config.view_dir}/${partial}${config.tpl_ext}`, 'utf8');
			if(tmplSource) {
				Handlebars.registerPartial(partial, tmplSource);
				template = Handlebars.partials[partial];
				Handlebars.partials[partial] = null; //for test
			}
		}
